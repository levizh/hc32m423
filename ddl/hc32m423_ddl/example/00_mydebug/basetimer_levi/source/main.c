...
void Timer0GCmp_IrqHandler(void)
{
    Timer0CompareIrqCallback();
}
...
int32_t main(void)
{
    ...
    stcIrqRegiCfg.enIRQn = Int027_IRQn;//TIMER0_IRQn;
    stcIrqRegiCfg.enIntSrc = INT_TMR0_GCMP;//TIMER0_SOURCE;
//    stcIrqRegiCfg.pfnCallback = &Timer0CompareIrqCallback;
//    INTC_IrqRegistration(&stcIrqRegiCfg);
    INTC_ShareIrqCmd(stcIrqRegiCfg.enIntSrc, Enable);
